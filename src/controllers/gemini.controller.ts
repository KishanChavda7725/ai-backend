...
export const geminiSearchHandler = async (req: Request, res: Response) => {
  try {
    ...
    try {
      if (image && prompt) {
        ...
        if (!fs.existsSync(imagePath)) {
          ...
          return res.status(400).json({ error: 'Uploaded image not found on server.' });
        }
    ...
    //   console.log('Gemini raw response:', JSON.stringify(result, null, 2));
      ...
      return res.status(422).json({ error: 'Gemini API error', details: apiError instanceof Error ? apiError.message : apiError });
    }
